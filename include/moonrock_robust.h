...
#include <stdbool.h>
#include <X11/Xlib.h>
#include "wm_compat.h"  // For AuraWM and Client structs (needed by session save/restore)
...
//
// The user can restart aura-wm to attempt to restore full compositing.
...
//
// The default path is ~/.config/aura-wm/session.state
...
//
// Parameters:
//   wm   — pointer to the global AuraWM state (for reading window info)
//   path — file path to write to (NULL for default ~/.config/aura-wm/session.state)
//
...
//
// Parameters:
//   wm   — pointer to the global AuraWM state (for moving/resizing windows)
